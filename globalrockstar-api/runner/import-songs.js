...
Contest.findById(contestId).exec(function(err, contest) {
    ...
    var csvStream = csv({
        ...
        var views = data[3] && data[3].replace(/\s/g, '') != "" ? parseInt(data[3]) : 0;
        var likes = data[4] && data[4].replace(/\s/g, '') != "" ? parseInt(data[4]) : 0;
        var finalChart2013 = data[5] && data[5].replace(/\s/g, '') != "" ? parseInt(data[5]) : 0;
        var nationalChart2013 = data[6] && data[6].replace(/\s/g, '') != "" ? parseInt(data[6]) : 0;
        ...
        payload.contest = payload.legacy.entry2013 ? contest : null ;
